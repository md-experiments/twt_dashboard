...
##### TABLES PARAMS ##### 
# AUTHOR table
col_entity='ner_company' #'original_user'
default_category='us' #'ai' --> needs to be lowercase
# BODY table
sort_col='date_pres' # 'Favs'
sort_ascending=False

df_tpc=pd.read_csv(f'{path}{pattern}_topics.csv', index_col=0)
selected_topics=[tpc.lower() for tpc in df_tpc.index.values]

df_aut=pd.read_csv(f'{path}{pattern}_authors.csv', index_col=0)
...
def update_bar_authors(list_from_click):
    ...
    data=[
            dict(
                    x=list(df_aut[df_aut.term==hashtag.lower()][col_entity].iloc[:authors_limit].values),
...
def update_table(list_from_click):
    ...
    df_out=df_out[indicator].sort_values(sort_col, ascending=sort_ascending)
